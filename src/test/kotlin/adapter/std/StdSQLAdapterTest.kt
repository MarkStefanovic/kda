...
class StdSQLAdapterTest {
    ...
    fun test_createTable() {
        ...
        val expected = "CREATE TABLE sales.customer (customer_id INT NOT NULL, first_name TEXT NULL, " +
    ...
    fun test_add() {
        ...
        val expected = "INSERT INTO sales.customer (customer_id, first_name, last_name) " +
    ...
    fun test_delete_w_multi_pk_cols() {
        ...
        val sql = pgSQLAdapter.delete(table = table, primaryKeyValues = rows)
        val expected = "WITH d (first_name, last_name) AS (VALUES ('Mark', 'Stefanovic'), ('Bob', 'Smith')) " +
    ...
    fun test_update() {
        ...
        val rows = Rows.of(
            Row.of("id" to IntValue(1)) to Row.of(
                "customer_id" to IntValue(1),
                "first_name" to StringValue("Mark", maxLength = 40),
                "last_name" to StringValue("Stefanovic", maxLength = 40),
            ),
            Row.of("id" to IntValue(2)) to Row.of(
                "customer_id" to IntValue(2),
                "first_name" to StringValue("Bob", maxLength = 40),
                "last_name" to StringValue("Smith", maxLength = 40),
            ),
            Row.of("id" to IntValue(3)) to Row.of(
                "customer_id" to IntValue(3),
                "first_name" to StringValue("Olive", maxLength = 40),
                "last_name" to StringValue("Oil", maxLength = 40),
            ),
        ...
        val expected = "WITH u (customer_id, first_name, last_name) AS " +
            "(VALUES (1, 'Mark', 'Stefanovic'), (2, 'Bob', 'Smith'), (3, 'Olive', 'Oil')) " +
    ...
    fun test_select_w_single_pk_col() {
        ...
        val expected = "SELECT customer_id, first_name, last_name FROM sales.customer WHERE customer_id IN (1, 2, 3)"
        assertEquals(expected = expected, actual = sql)
    }
    ...
    fun test_select_w_multi_pk_cols() {
        ...
        val sql = pgSQLAdapter.select(table = table, primaryKeyValues = rows)
        val expected = "WITH v (first_name, last_name) AS (VALUES ('Mark', 'Stefanovic'), ('Bob', 'Smith'), ('Olive', 'Oil')) " +
