...
class StdSQLAdapter(private val impl: SQLAdapterImplDetails) : SQLAdapter {
    ...
    override fun delete(table: Table, primaryKeyValues: Rows): String {
        ...
        return if (table.sortedPrimaryKeyFieldNames.count() > 1) {
            val whereClause = table.sortedPrimaryKeyFieldNames.map { fld -> impl.wrapName(fld) }
                .joinToString(" AND ") { fld -> "t.$fld = d.$fld" }
            val pkColNames = pkColNameCSV(table)
            val valuesCSV = primaryKeyValues.values.joinToString(", ") { row ->
            ...
            val valuesCSV = primaryKeyValues.values.flatMap { row ->
    ...
    override fun update(table: Table, rows: Rows): String {
        ...
        val setClause =
            nonPKCols.sorted().joinToString(", ") { fld -> "t.${impl.wrapName(fld)} = u.${impl.wrapName(fld)}" }
        val tableName = fullTableName(schema = table.schema, table = table.name)
        return "WITH u ($colNameCSV) AS (VALUES ${impl.valuesExpression(fieldNames = fieldNames, rows = rows)}) " +
    ...
    override fun select(table: Table, primaryKeyValues: Rows): String {
        ...
        return if (pkCols.count() > 1) {
            ...
            val whereClause =
                pkCols.map { fld -> impl.wrapName(fld) }.joinToString(" AND ") { fld -> "t.$fld = d.$fld" }
            ...
            val valuesCSV = primaryKeyValues.values.joinToString(", ") { row -> impl.wrapValue(row.value(pkCol)) }
    ...
    private fun fullTableName(schema: String?, table: String): String =
        if (schema == null)
            ...
            "${impl.wrapName(schema)}.${impl.wrapName(table)}"
